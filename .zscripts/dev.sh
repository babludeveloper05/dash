...
# db:push is skipped — the app uses FastAPI + SQLAlchemy (mini-services/api/)
# for its database, not Prisma. Running db:push would try to drop the
# FastAPI-created tables. The Prisma client in src/lib/db.ts is unused by
# the app but kept for compatibility.
log_step_start "Skipping db:push (FastAPI manages the DB)"
echo "[BUN] Skipping db:push — DB is managed by FastAPI/SQLAlchemy"
log_step_end "Skipping db:push (FastAPI manages the DB)"
